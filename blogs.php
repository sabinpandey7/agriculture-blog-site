  ...
  <body>
    ...
    <div style="min-height:90vh;">
         <div class="container py-5">
          <div>
           ...
           <div class="list-group list-flush">
            ...
            <?php
              $sql= "select * from posts where type='blog' and status=1 ;";
              $result=mysqli_query($conn,$sql);
              $total = mysqli_num_rows($result);
              $pageCount =ceil(($total)/7);
              $sql = "select * from posts where type='blog'  and status=1 order by id DESC limit 5 offset $offset";
              $result=mysqli_query($conn,$sql);
      ...
      <div class="container d-flex justify-content-between">
        <?php 
          if($page>1)
           echo '<a href="./blogs.php?page='.($page-1).'" class="btn btn-success">Prev</a>';
         ?>
        <?php 
          if($page<$pageCount)
           echo '<a <a href="./blogs.php?page='.($page+1).'"  type="button" class="btn btn-success" >Next</a>';
         ?>
      </div>
    </div>
            </div>
